...
## Reference genome
<<<<<<< HEAD
The segment data used in this tutorial was done on the Human reference genome hg18. Note that this file must include both __cytoband__ information and __stain__ information. Our document titled __How to obtain Cytoband and Stain Information__ contains information on how to create this object in the right format.

_The objects to input into the package:_ `hg18.ucsctrack`
```{r refGen, message=FALSE, warning=FALSE}
data("hg18.ucsctrack")
=======
As mentioned previously, the segment data used in this tutorial was done on the Human reference genome hg18. So we need to download this file. Note that this file must include both __cytoband__ information and __stain__ information. We use the ```biovizBase``` package for this.
```{r refGen, message=FALSE, warning=FALSE}
